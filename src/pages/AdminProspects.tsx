    ...
    <div style={{ minHeight:'100vh', background:'transparent' }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'16px' }}>
        ...
        <div className='flex gap-2 mb-4'>
          <button onClick={() => setTab('prospects')} style={{ padding:'8px 16px', borderRadius:100, border:'none', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight:600, cursor:'pointer', background:tab==='prospects'?'#2DD4B8':'rgba(255,255,255,0.06)', color:tab==='prospects'?'#fff':'rgba(242,237,228,0.55)' }}>
            Prospects
          </button>
          <button onClick={() => setTab('search')} style={{ padding:'8px 16px', borderRadius:100, border:'none', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight:600, cursor:'pointer', background:tab==='search'?'#2DD4B8':'rgba(255,255,255,0.06)', color:tab==='search'?'#fff':'rgba(242,237,228,0.55)', display:'flex', alignItems:'center', gap:4 }}>
            <Search size={12}/> Chercher
          </button>
          <button onClick={() => setShowSectors(true)} style={{ padding:'8px 16px', borderRadius:100, border:'none', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight:600, cursor:'pointer', background:'rgba(255,255,255,0.06)', color:'rgba(242,237,228,0.55)', display:'flex', alignItems:'center', gap:4 }}>
            📊 Secteurs
          </button>
        </div>
            ...
            <div style={{ padding:24, background:'rgba(255,255,255,0.10)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'28px' }}>
              ...
              <div className='flex flex-col gap-3'>
                <div className='flex flex-col sm:flex-row gap-3'>
                  <select value={searchContinent} onChange={e => { setSearchContinent(e.target.value); setSearchCountry(''); }} style={{ flex:1, padding:'10px 14px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#F2EDE4', cursor:'pointer', outline:'none' }}>
                    <option value=''>🌍 Continent (optionnel)</option>
                    {Object.keys(CONTINENTS).map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                  <select value={searchCountry} onChange={e => setSearchCountry(e.target.value)} style={{ flex:1, padding:'10px 14px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#F2EDE4', cursor:'pointer', outline:'none' }}>
                  ...
                  <input placeholder='📍 Ville (optionnel, ex: Lyon)' value={searchCity} onChange={e => setSearchCity(e.target.value)} style={{ flex:1, padding:'10px 14px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#F2EDE4', outline:'none' }} />
                </div>
                <div className='flex flex-col gap-3'>
                  <div>
                    <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'rgba(242,237,228,0.28)', display:'block', marginBottom:8, textTransform:'uppercase', letterSpacing:1 }}>Types de commerce * (multi-selection)</label>
                    <div className='flex flex-wrap gap-2'>
                      {BT.map(t => (
                        <button key={t} onClick={() => toggleSearchType(t)} style={{ padding:'6px 14px', borderRadius:'100px', border:'1px solid', borderColor:searchTypes.includes(t)?'#2DD4B8':'rgba(255,255,255,0.12)', background:searchTypes.includes(t)?'rgba(13,138,111,0.12)':'rgba(255,255,255,0.06)', color:searchTypes.includes(t)?'#2DD4B8':'rgba(242,237,228,0.55)', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight:searchTypes.includes(t)?600:400, cursor:'pointer', transition:'all 0.2s' }}>
                          {searchTypes.includes(t) ? '✓ ' : ''}{t}
                        </button>
                      ))}
                    </div>
                  </div>
                  {searchTypes.includes('Autre') && <input placeholder='Type personnalise...' value={customType} onChange={e => setCustomType(e.target.value)} style={{ padding:'10px 14px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#F2EDE4', outline:'none' }} />}
                  <div className='flex items-center gap-3 flex-wrap'>
                    <div className='flex items-center gap-2' style={{ padding:'10px 14px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'12px', flex:'0 0 auto' }}>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'rgba(242,237,228,0.28)', whiteSpace:'nowrap' }}>Max resultats:</label>
                      <input type='number' min={5} max={500} value={maxResults} onChange={e => setMaxResults(Number(e.target.value))} style={{ width:60, background:'transparent', border:'none', outline:'none', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#F2EDE4', textAlign:'center' }} />
                    </div>
                    <button onClick={() => setFetchPhone(!fetchPhone)} style={{ padding:'10px 14px', background: fetchPhone ? 'rgba(13,138,111,0.12)' : 'rgba(255,255,255,0.06)', border:'1px solid', borderColor: fetchPhone ? '#2DD4B8' : 'rgba(255,255,255,0.12)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight: fetchPhone ? 600 : 400, cursor:'pointer', display:'flex', alignItems:'center', gap:6, color: fetchPhone ? '#2DD4B8' : 'rgba(242,237,228,0.55)', flex:'0 0 auto' }}>
                    ...
                    <button onClick={() => setForceResearch(!forceResearch)} style={{ padding:'10px 14px', background: forceResearch ? 'rgba(232,115,90,0.12)' : 'rgba(255,255,255,0.06)', border:'1px solid', borderColor: forceResearch ? '#F07067' : 'rgba(255,255,255,0.12)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight: forceResearch ? 600 : 400, cursor:'pointer', display:'flex', alignItems:'center', gap:6, color: forceResearch ? '#F07067' : 'rgba(242,237,228,0.55)', flex:'0 0 auto' }}>
                  ...
                  {searchResults.map(r => (
                    <div key={r.google_place_id} className='flex items-center gap-4' style={{ padding:'12px 16px', background:r.has_website?'transparent':'rgba(13,138,111,0.04)', borderRadius:'12px', border:'1px solid rgba(255,255,255,0.12)' }}>
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:12, color:'rgba(242,237,228,0.28)', fontFamily:"'Outfit', sans-serif" }}>{r.address}</div>
                        {r.phone && <div style={{ fontSize:12, color:'rgba(242,237,228,0.55)', fontFamily:"'Outfit', sans-serif" }}>{r.phone}</div>}
                      ...
                      <span style={{ padding:'6px 14px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'100px', fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#2DD4B8', fontWeight:600 }}>
            ...
            <div style={{ background:'rgba(255,255,255,0.10)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'28px', padding:20 }}>
              ...
              {showChunkHistory && searchChunks.length > 0 && (
                <div style={{ marginTop:16, display:'flex', flexDirection:'column', gap:6 }}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr auto auto auto auto', gap:8, padding:'8px 12px', fontFamily:"'Outfit', sans-serif", fontSize:11, color:'rgba(242,237,228,0.28)', textTransform:'uppercase', letterSpacing:1, fontWeight:600 }}>
                  ...
                  {searchChunks.map(c => (
                    <div key={c.id} style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr auto auto auto auto', gap:8, padding:'8px 12px', background:'rgba(255,255,255,0.06)', borderRadius:'12px', border:'1px solid rgba(255,255,255,0.12)', alignItems:'center' }}>
                      ...
                      <span style={{ fontFamily:"'Outfit', sans-serif", fontSize:11, color:'rgba(242,237,228,0.28)', whiteSpace:'nowrap' }}>{new Date(c.created_at).toLocaleDateString('fr-FR', { day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' })}</span>
                  ...
                  <div style={{ padding:'10px 12px', background:'rgba(13,138,111,0.04)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:12, color:'rgba(242,237,228,0.55)', display:'flex', justifyContent:'space-between' }}>
              ...
              {showChunkHistory && searchChunks.length === 0 && (
                <p style={{ marginTop:12, fontFamily:"'Outfit', sans-serif", fontSize:13, color:'rgba(242,237,228,0.28)' }}>Aucune recherche effectuée pour le moment.</p>
              )}
            </div>
          </div>
        )}
        ...
        {tab === 'prospects' && (
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <div className='flex flex-col gap-2'>
              <div className='flex items-center gap-2' style={{ padding:'8px 12px', background:'rgba(255,255,255,0.06)', borderRadius:16, border:'1px solid rgba(255,255,255,0.12)' }}>
                <Search size={14} style={{ color:'rgba(242,237,228,0.28)', flexShrink:0 }} />
                <input placeholder='Rechercher...' value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ flex:1, background:'transparent', border:'none', outline:'none', fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#F2EDE4' }} />
              </div>
              <div className='flex gap-2'>
                <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ flex:1, padding:'8px 10px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:16, fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#F2EDE4', cursor:'pointer', outline:'none' }}>
                  <option value='all'>Tous statuts</option>
                  {Object.entries(SL).map(([k,v]) => <option key={k} value={k}>{v}</option>)}
                </select>
                <select value={websiteFilter} onChange={e => setWebsiteFilter(e.target.value as any)} style={{ flex:1, padding:'8px 10px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:16, fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#F2EDE4', cursor:'pointer', outline:'none' }}>
                  <option value='all'>Tous (site)</option>
                  <option value='no_website'>🔒 Sans site</option>
                  <option value='has_website'>🌐 Avec site</option>
                </select>
                <button onClick={() => setShowManualAdd(true)} style={{ padding:'8px 12px', background:'rgba(255,255,255,0.10)', color:'rgba(242,237,228,0.55)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:16, cursor:'pointer', flexShrink:0 }}>
                  <Plus size={14}/>
                </button>
                <button onClick={fetchProspects} style={{ padding:'8px 12px', background:'rgba(255,255,255,0.10)', color:'rgba(242,237,228,0.55)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:16, cursor:'pointer', flexShrink:0 }}>
                  <RefreshCw size={14}/>
                </button>
              </div>
              {/* Advanced filters toggle + saved searches */}
              <div className='flex gap-2 items-center'>
                <button onClick={() => setShowAdvancedFilters(!showAdvancedFilters)} style={{ padding:'6px 12px', borderRadius:100, border:'1px solid rgba(255,255,255,0.12)', background: showAdvancedFilters ? 'rgba(13,138,111,0.1)' : 'rgba(255,255,255,0.06)', color: showAdvancedFilters ? '#2DD4B8' : 'rgba(242,237,228,0.55)', fontFamily:"'Outfit', sans-serif", fontSize:11, cursor:'pointer', display:'flex', alignItems:'center', gap:4 }}>
                  <Target size={11}/> Filtres avancés {showAdvancedFilters ? '▲' : '▼'}
                </button>
                <button onClick={saveSearch} disabled={savingSearch} style={{ padding:'6px 12px', borderRadius:100, border:'1px solid rgba(255,255,255,0.12)', background:'rgba(255,255,255,0.06)', color:'rgba(242,237,228,0.55)', fontFamily:"'Outfit', sans-serif", fontSize:11, cursor:'pointer' }}>
                  💾 Sauvegarder
                </button>
                {savedSearches.length > 0 && (
                  <select onChange={e => { const s = savedSearches.find(x => x.id === e.target.value); if (s) loadSearch(s); }} defaultValue='' style={{ padding:'6px 10px', borderRadius:100, border:'1px solid rgba(255,255,255,0.12)', background:'rgba(255,255,255,0.06)', fontFamily:"'Outfit', sans-serif", fontSize:11, color:'rgba(242,237,228,0.55)', cursor:'pointer', outline:'none' }}>
              ...
              {showAdvancedFilters && (
                <div style={{ padding:12, background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:16, display:'flex', flexDirection:'column', gap:10 }}>
                  <div className='flex items-center gap-3 flex-wrap'>
                    <div style={{ flex:'1 1 200px' }}>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:10, color:'rgba(242,237,228,0.28)', textTransform:'uppercase', letterSpacing:0.5, display:'block', marginBottom:4 }}>Score minimum: {scoreMin}</label>
                    ...
                    <div style={{ flex:'1 1 150px' }}>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:10, color:'rgba(242,237,228,0.28)', textTransform:'uppercase', letterSpacing:0.5, display:'block', marginBottom:4 }}>Séquence</label>
                      <select value={sequenceFilter} onChange={e => setSequenceFilter(e.target.value as any)} style={{ width:'100%', padding:'6px 8px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:8, fontFamily:"'Outfit', sans-serif", fontSize:12, outline:'none' }}>
                    ...
                    <div style={{ flex:'1 1 150px' }}>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:10, color:'rgba(242,237,228,0.28)', textTransform:'uppercase', letterSpacing:0.5, display:'block', marginBottom:4 }}>Dernière interaction (jours)</label>
                      <select value={lastInteractionDays ?? ''} onChange={e => setLastInteractionDays(e.target.value ? Number(e.target.value) : null)} style={{ width:'100%', padding:'6px 8px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:8, fontFamily:"'Outfit', sans-serif", fontSize:12, outline:'none' }}>
                  ...
                  {allSectors.length > 0 && (
                    <div>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:10, color:'rgba(242,237,228,0.28)', textTransform:'uppercase', letterSpacing:0.5, display:'block', marginBottom:4 }}>Secteurs</label>
                      <div className='flex flex-wrap gap-1'>
                        {allSectors.map(s => (
                          <button key={s} onClick={() => setSectorFilter(prev => prev.includes(s) ? prev.filter(x => x !== s) : [...prev, s])} style={{ padding:'3px 10px', borderRadius:100, border:'1px solid', borderColor: sectorFilter.includes(s) ? '#2DD4B8' : 'rgba(255,255,255,0.12)', background: sectorFilter.includes(s) ? 'rgba(13,138,111,0.1)' : 'transparent', color: sectorFilter.includes(s) ? '#2DD4B8' : 'rgba(242,237,228,0.55)', fontFamily:"'Outfit', sans-serif", fontSize:11, cursor:'pointer' }}>{s}</button>
                        ))}
                      </div>
                    </div>
                  )}
            ...
            <div style={{ display:'flex', justifyContent:'flex-end' }}>
              ...
              ])} style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 14px', background:'rgba(255,255,255,0.10)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:11, cursor:'pointer', color:'rgba(242,237,228,0.55)' }}>
            ...
            <div style={{ padding:'10px 14px', background:'rgba(212,165,90,0.08)', border:'1px solid rgba(212,165,90,0.3)', borderRadius:16, display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:8 }}>
              <span style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'rgba(242,237,228,0.55)', display:'flex', alignItems:'center', gap:4 }}>
            ...
            {selectedIds.size > 0 && (
              <div style={{ padding:'10px 14px', background:'rgba(13,138,111,0.08)', border:'1px solid rgba(13,138,111,0.3)', borderRadius:16, display:'flex', flexWrap:'wrap', alignItems:'center', gap:8 }}>
                ...
                <button onClick={() => setSelectedIds(new Set())} style={{ padding:'6px 12px', background:'transparent', border:'1px solid rgba(255,255,255,0.12)', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:11, color:'rgba(242,237,228,0.55)', cursor:'pointer' }}>Désélectionner</button>
            ...
            {loading ? <div className='text-center py-20' style={{ color:'rgba(242,237,228,0.28)', fontFamily:"'Outfit', sans-serif" }}>Chargement...</div>
            : filteredProspects.length === 0 ? <div className='text-center py-20' style={{ color:'rgba(242,237,228,0.28)', fontFamily:"'Outfit', sans-serif" }}>Aucun prospect — utilise Chercher des prospects.</div>
            : (
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
              {[
                ...
                <div key={field.key} style={{ gridColumn:field.full ? '1 / -1' : undefined }}>
                  <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'rgba(242,237,228,0.28)', display:'block', marginBottom:4 }}>{field.label}</label>
                  <input value={(manualForm as any)[field.key]} onChange={e => setManualForm(prev => ({ ...prev, [field.key]: e.target.value }))} style={{ width:'100%', padding:'10px 14px', background:'rgba(255,255,255,0.85)', border:'1px solid rgba(255,255,255,0.5)', borderRadius:12, fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#F2EDE4', outline:'none', boxSizing:'border-box', boxShadow:'inset 0 1px 2px rgba(0,0,0,0.04)' }} />
                </div>
              ))}
            </div>
            <div className='flex gap-3 mt-6'>
              <button onClick={() => setShowManualAdd(false)} style={{ flex:1, padding:12, background:'rgba(255,255,255,0.25)', border:'1px solid rgba(255,255,255,0.4)', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:14, cursor:'pointer', color:'rgba(242,237,228,0.55)', backdropFilter:'blur(8px)' }}>Annuler</button>
...
const ProspectTable = ({ prospects, selectedIds, onToggleSelect, onToggleSelectAll, onDelete, onUpdateEmail, onUpdateStatus, onViewDetail }: {
  ...
  <div style={{ background:'rgba(255,255,255,0.10)', backdropFilter:'blur(20px)', borderRadius:'12px', border:'1px solid rgba(255,255,255,0.12)', overflow:'hidden' }}>
      ...
      {prospects.map(p => (
        <div key={p.id} className='flex items-start gap-3 p-4' style={{ borderBottom:'1px solid rgba(0,0,0,0.04)', cursor:'pointer' }} onClick={() => onViewDetail(p)}>
          ...
          <div style={{ flex:1 }}>
            ...
            <p style={{ fontSize:12, color:'rgba(242,237,228,0.28)', margin:0, fontFamily:"'Outfit', sans-serif" }}>{p.city} - {p.has_website ? 'Site OK' : 'Sans site'} - {p.email_count}x</p>
          </div>
        </div>
      ))}
    </div>
    ...
    <tr style={{ borderBottom:'1px solid rgba(0,0,0,0.04)', background:selected?'rgba(13,138,111,0.04)':'transparent', cursor:'pointer' }} onClick={onViewDetail}>
      ...
      <td className='px-4 py-3'>
        ...
        <div className='flex items-center gap-1'>
          {p.business_type && <span style={{ fontSize:11, color:'rgba(242,237,228,0.28)' }}>{p.business_type}</span>}
      ...
      <td className='px-4 py-3' style={{ color:'rgba(242,237,228,0.55)', fontSize:13 }}>
        {p.contact_name || '—'}
        {p.phone && <div style={{ fontSize:11, color:'rgba(242,237,228,0.28)' }}>{p.phone}</div>}
      </td>
      <td className='px-4 py-3'>
        {editingEmail ? (
          <div className='flex items-center gap-1'>
            ...
            <button onClick={() => setEditingEmail(false)} style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(242,237,228,0.28)' }}><X size={14}/></button>
          </div>
        ) : (
          <div className='flex items-center gap-2'>
            <span style={{ fontSize:13, color:p.email?'rgba(242,237,228,0.55)':'rgba(242,237,228,0.20)', fontStyle:p.email?'normal':'italic' }}>{p.email || 'Pas d\'email'}</span>
      ...
      <td className='px-4 py-3' style={{ color:'rgba(242,237,228,0.55)', fontSize:13 }}>
        <div>{p.city || '—'}</div>
        <span style={{ fontSize:10, color:'rgba(242,237,228,0.28)', fontWeight:600 }}>{LANG_LABELS[p.language || COUNTRY_LANG[p.country || ''] || 'en'] || p.language}</span>
      </td>
      <td className='px-4 py-3'>
        {p.has_website
          ? <span style={{ display:'flex', alignItems:'center', gap:4, fontSize:12, color:'rgba(242,237,228,0.28)' }}><Globe size={12}/> A un site</span>
      ...
      <td className='px-4 py-3' style={{ fontSize:12, color:'rgba(242,237,228,0.28)' }}>{p.email_count}x</td>
      <td className='px-4 py-3'>
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', backdropFilter:'blur(8px)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ width:'100%', maxWidth:800, background:'rgba(255,255,255,0.06)', borderRadius:'28px', overflow:'hidden', maxHeight:'92vh', display:'flex', flexDirection:'column' }}>
        <div style={{ padding:'20px 28px', borderBottom:'1px solid rgba(255,255,255,0.12)', display:'flex', alignItems:'center', justifyContent:'space-between', background:'rgba(255,255,255,0.10)' }}>
          ...
          <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(242,237,228,0.55)' }}><X size={20}/></button>
        </div>
        <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
          <div style={{ width:200, borderRight:'1px solid rgba(255,255,255,0.12)', overflowY:'auto', background:'rgba(255,255,255,0.06)', flexShrink:0 }}>
            {prospects.map((p,i) => {
          ...
          <div style={{ flex:1, overflowY:'auto', padding:28 }}>
            {generatedEmails[activeIdx] && (() => {
              ...
              if (e.loading) return (
                <div style={{ display:'flex', alignItems:'center', gap:12, color:'rgba(242,237,228,0.55)', fontFamily:"'Outfit', sans-serif", paddingTop:40 }}>
              ...
              return (
                <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                  <div style={{ padding:'8px 12px', background:'rgba(13,138,111,0.06)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:12, color:'rgba(242,237,228,0.55)' }}>
        ...
        <div style={{ padding:'16px 28px', borderTop:'1px solid rgba(255,255,255,0.12)', display:'flex', alignItems:'center', justifyContent:'space-between', background:'rgba(255,255,255,0.06)' }}>
          <p style={{ fontFamily:"'Outfit', sans-serif", fontSize:13, color:'rgba(242,237,228,0.55)', margin:0 }}>{ready.length} email(s) prets</p>
          <div className='flex gap-3'>
            <button onClick={onClose} style={{ padding:'10px 20px', background:'transparent', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'100px', fontFamily:"'Outfit', sans-serif", fontSize:13, cursor:'pointer', color:'rgba(242,237,228,0.55)' }}>Annuler</button>
