      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'16px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(110px, 1fr))', gap:8, marginBottom:16 }}>
          {[
            ...
            <div key={s.label} style={{ padding:'10px 12px', background:'rgba(255,255,255,0.45)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:16, display:'flex', alignItems:'center', gap:8 }}>
              <s.icon size={14} style={{ color:'#2DD4B8', flexShrink:0 }}/>
              <div style={{ minWidth:0 }}>
                <div style={{ fontFamily:"'Playfair Display', serif", fontSize:16, color:'#2a2722', lineHeight:1.1 }}>{s.value}</div>
                <div style={{ fontFamily:"'Outfit', sans-serif", fontSize:9, color:'#9a9490', textTransform:'uppercase', letterSpacing:0.5, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{s.label}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className='flex gap-2 mb-4'>
          <button onClick={() => setTab('prospects')} style={{ padding:'8px 16px', borderRadius:100, border:'none', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight:600, cursor:'pointer', background:tab==='prospects'?'#2DD4B8':'rgba(255,255,255,0.5)', color:tab==='prospects'?'#fff':'#6b6560' }}>
            Prospects
          </button>
          <button onClick={() => setTab('search')} style={{ padding:'8px 16px', borderRadius:100, border:'none', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight:600, cursor:'pointer', background:tab==='search'?'#2DD4B8':'rgba(255,255,255,0.5)', color:tab==='search'?'#fff':'#6b6560', display:'flex', alignItems:'center', gap:4 }}>
            <Search size={12}/> Chercher
          </button>
          <button onClick={() => setShowSectors(true)} style={{ padding:'8px 16px', borderRadius:100, border:'none', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight:600, cursor:'pointer', background:'rgba(255,255,255,0.5)', color:'#6b6560', display:'flex', alignItems:'center', gap:4 }}>
            📊 Secteurs
          </button>
        </div>
        ...
        {tab === 'search' && (
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <div style={{ padding:24, background:'rgba(255,255,255,0.45)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'28px' }}>
              <h3 style={{ fontFamily:"'Playfair Display', serif", fontSize:16, color:'#2a2722', margin:'0 0 8px' }}>Recherche Google Maps — Mondiale</h3>
              <p style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#9a9490', margin:'0 0 16px' }}>Choisis un continent, un pays, une ville — ou combine-les. Seul le type est obligatoire.</p>
              <div className='flex flex-col gap-3'>
                <div className='flex flex-col sm:flex-row gap-3'>
                  <select value={searchContinent} onChange={e => { setSearchContinent(e.target.value); setSearchCountry(''); }} style={{ flex:1, padding:'10px 14px', background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#2a2722', cursor:'pointer', outline:'none' }}>
                    <option value=''>🌍 Continent (optionnel)</option>
                    {Object.keys(CONTINENTS).map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                  <select value={searchCountry} onChange={e => setSearchCountry(e.target.value)} style={{ flex:1, padding:'10px 14px', background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#2a2722', cursor:'pointer', outline:'none' }}>
                  ...
                  <input placeholder='📍 Ville (optionnel, ex: Lyon)' value={searchCity} onChange={e => setSearchCity(e.target.value)} style={{ flex:1, padding:'10px 14px', background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#2a2722', outline:'none' }} />
                </div>
                <div className='flex flex-col gap-3'>
                  <div>
                    <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#9a9490', display:'block', marginBottom:8, textTransform:'uppercase', letterSpacing:1 }}>Types de commerce * (multi-selection)</label>
                    <div className='flex flex-wrap gap-2'>
                      {BT.map(t => (
                        <button key={t} onClick={() => toggleSearchType(t)} style={{ padding:'6px 14px', borderRadius:'100px', border:'1px solid', borderColor:searchTypes.includes(t)?'#2DD4B8':'rgba(0,0,0,0.08)', background:searchTypes.includes(t)?'rgba(13,138,111,0.12)':'rgba(255,255,255,0.5)', color:searchTypes.includes(t)?'#2DD4B8':'#6b6560', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight:searchTypes.includes(t)?600:400, cursor:'pointer', transition:'all 0.2s' }}>
                          {searchTypes.includes(t) ? '✓ ' : ''}{t}
                        </button>
                      ))}
                    </div>
                  </div>
                  {searchTypes.includes('Autre') && <input placeholder='Type personnalise...' value={customType} onChange={e => setCustomType(e.target.value)} style={{ padding:'10px 14px', background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#2a2722', outline:'none' }} />}
                  <div className='flex items-center gap-3 flex-wrap'>
                    <div className='flex items-center gap-2' style={{ padding:'10px 14px', background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'12px', flex:'0 0 auto' }}>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#9a9490', whiteSpace:'nowrap' }}>Max resultats:</label>
                      <input type='number' min={5} max={500} value={maxResults} onChange={e => setMaxResults(Number(e.target.value))} style={{ width:60, background:'transparent', border:'none', outline:'none', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#2a2722', textAlign:'center' }} />
                    </div>
                    <button onClick={() => setFetchPhone(!fetchPhone)} style={{ padding:'10px 14px', background: fetchPhone ? 'rgba(13,138,111,0.12)' : 'rgba(255,255,255,0.5)', border:'1px solid', borderColor: fetchPhone ? '#2DD4B8' : 'rgba(0,0,0,0.08)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight: fetchPhone ? 600 : 400, cursor:'pointer', display:'flex', alignItems:'center', gap:6, color: fetchPhone ? '#2DD4B8' : '#6b6560', flex:'0 0 auto' }}>
                    ...
                    <button onClick={() => setForceResearch(!forceResearch)} style={{ padding:'10px 14px', background: forceResearch ? 'rgba(232,115,90,0.12)' : 'rgba(255,255,255,0.5)', border:'1px solid', borderColor: forceResearch ? '#F07067' : 'rgba(0,0,0,0.08)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight: forceResearch ? 600 : 400, cursor:'pointer', display:'flex', alignItems:'center', gap:6, color: forceResearch ? '#F07067' : '#6b6560', flex:'0 0 auto' }}>
                      <RefreshCw size={13}/> {forceResearch ? '✓ Forcer re-recherche' : 'Forcer re-recherche'}
                    </button>
                    <button onClick={() => handleSearch(false)} disabled={searching} style={{ padding:'10px 24px', background:'#2DD4B8', color:'#fff', border:'none', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:14, fontWeight:600, cursor:searching?'not-allowed':'pointer', display:'flex', alignItems:'center', gap:6, opacity:searching?0.7:1, whiteSpace:'nowrap', flex:'0 0 auto' }}>
            ...
            {searchResults && (
              <div style={{ background:'rgba(255,255,255,0.45)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'28px', padding:20 }}>
                <div className='flex items-center justify-between mb-4'>
                  <h4 style={{ fontFamily:"'Playfair Display', serif", fontSize:15, color:'#2a2722', margin:0 }}>{searchResults.length} resultats</h4>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {searchResults.map(r => (
                    <div key={r.google_place_id} className='flex items-center gap-4' style={{ padding:'12px 16px', background:r.has_website?'transparent':'rgba(13,138,111,0.04)', borderRadius:'12px', border:'1px solid rgba(0,0,0,0.08)' }}>
                      <div style={{ flex:1 }}>
                        <div className='flex items-center gap-2'>
                          <span style={{ fontWeight:600, fontFamily:"'Outfit', sans-serif", color:'#2a2722' }}>{r.business_name}</span>
                          {r.rating && <span style={{ fontSize:11, color:'#9a9490', display:'flex', alignItems:'center', gap:2 }}><Star size={10}/> {r.rating} ({r.review_count})</span>}
                          {r.has_website ? <Globe size={12} style={{ color:'#9a9490' }}/> : <GlobeLock size={12} style={{ color:'#2DD4B8' }}/>}
                        </div>
                        <div style={{ fontSize:12, color:'#9a9490', fontFamily:"'Outfit', sans-serif" }}>{r.address}</div>
                        {r.phone && <div style={{ fontSize:12, color:'#6b6560', fontFamily:"'Outfit', sans-serif" }}>{r.phone}</div>}
                        {(r as any).email && <div style={{ fontSize:12, color:'#2DD4B8', fontFamily:"'Outfit', sans-serif", display:'flex', alignItems:'center', gap:4 }}><Mail size={11}/> {(r as any).email} <span style={{ fontSize:10, opacity:0.7 }}>({(r as any).email_confidence})</span></div>}
                      </div>
                      <span style={{ padding:'6px 14px', background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'100px', fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#2DD4B8', fontWeight:600 }}>
            ...
            {searchChunks.length > 0 && (
              <div style={{ background:'rgba(255,255,255,0.45)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'28px', padding:20 }}>
                <div className='flex items-center gap-2 mb-4'>
                  ...
                  <h4 style={{ fontFamily:"'Playfair Display', serif", fontSize:15, color:'#2a2722', margin:0 }}>Couverture par continent</h4>
                </div>
                <div className='flex flex-col gap-3'>
                    ...
                    return (
                      <div key={continent}>
                        <div className='flex items-center justify-between mb-1'>
                          <span style={{ fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#2a2722', fontWeight:600 }}>{continent}</span>
                          <span style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#6b6560' }}>{covered}/{total} pays · {totalResults} prospects</span>
                        </div>
                        <div style={{ height:8, background:'rgba(255,255,255,0.5)', borderRadius:99, overflow:'hidden' }}>
                          <div style={{ height:'100%', width:`${pct}%`, background: pct === 100 ? '#2DD4B8' : 'linear-gradient(90deg, #2DD4B8, #4da6d9)', borderRadius:99, transition:'width 0.5s ease' }}/>
                        </div>
            ...
            <div style={{ background:'rgba(255,255,255,0.45)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'28px', padding:20 }}>
              <button onClick={() => setShowChunkHistory(!showChunkHistory)} className='flex items-center justify-between w-full' style={{ background:'none', border:'none', cursor:'pointer', padding:0 }}>
                <div className='flex items-center gap-2'>
                  ...
                  <h4 style={{ fontFamily:"'Playfair Display', serif", fontSize:15, color:'#2a2722', margin:0 }}>Historique des recherches</h4>
                ...
                <span style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#9a9490' }}>{showChunkHistory ? '▲ Masquer' : '▼ Voir'}</span>
              </button>
              {showChunkHistory && searchChunks.length > 0 && (
                <div style={{ marginTop:16, display:'flex', flexDirection:'column', gap:6 }}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr auto auto auto auto', gap:8, padding:'8px 12px', fontFamily:"'Outfit', sans-serif", fontSize:11, color:'#9a9490', textTransform:'uppercase', letterSpacing:1, fontWeight:600 }}>
                    <span>Type</span><span>Pays</span><span>Ville</span><span>Résultats</span><span>Mode</span><span>Date</span><span></span>
                  </div>
                  {searchChunks.map(c => (
                    <div key={c.id} style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr auto auto auto auto', gap:8, padding:'8px 12px', background:'rgba(255,255,255,0.5)', borderRadius:'12px', border:'1px solid rgba(0,0,0,0.08)', alignItems:'center' }}>
                      <span style={{ fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#2a2722', fontWeight:600 }}>{c.business_type}</span>
                      <span style={{ fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#6b6560' }}>{c.country || '—'}</span>
                      <span style={{ fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#6b6560' }}>{c.city || '—'}</span>
                  ...
                  <div style={{ padding:'10px 12px', background:'rgba(13,138,111,0.04)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#6b6560', display:'flex', justifyContent:'space-between' }}>
                    ...
                    <span style={{ color:'#2DD4B8', fontWeight:600 }}>{searchChunks.reduce((s, c) => s + Number(c.cost_eur), 0).toFixed(3)}€</span>
                  </div>
                </div>
              )}
              {showChunkHistory && searchChunks.length === 0 && (
                <p style={{ marginTop:12, fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#9a9490' }}>Aucune recherche effectuée pour le moment.</p>
              )}
            </div>
          </div>
        )}
        ...
        {tab === 'prospects' && (
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <div className='flex flex-col gap-2'>
              <div className='flex items-center gap-2' style={{ padding:'8px 12px', background:'rgba(255,255,255,0.5)', borderRadius:16, border:'1px solid rgba(0,0,0,0.08)' }}>
                <Search size={14} style={{ color:'#9a9490', flexShrink:0 }} />
                <input placeholder='Rechercher...' value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ flex:1, background:'transparent', border:'none', outline:'none', fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#2a2722' }} />
              </div>
              <div className='flex gap-2'>
                <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ flex:1, padding:'8px 10px', background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:16, fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#2a2722', cursor:'pointer', outline:'none' }}>
                  <option value='all'>Tous statuts</option>
                  {Object.entries(SL).map(([k,v]) => <option key={k} value={k}>{v}</option>)}
                </select>
                <select value={websiteFilter} onChange={e => setWebsiteFilter(e.target.value as any)} style={{ flex:1, padding:'8px 10px', background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:16, fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#2a2722', cursor:'pointer', outline:'none' }}>
                  <option value='all'>Tous (site)</option>
                  <option value='no_website'>🔒 Sans site</option>
                  <option value='has_website'>🌐 Avec site</option>
                </select>
                <button onClick={() => setShowManualAdd(true)} style={{ padding:'8px 12px', background:'rgba(255,255,255,0.45)', color:'#6b6560', border:'1px solid rgba(0,0,0,0.08)', borderRadius:16, cursor:'pointer', flexShrink:0 }}>
                  <Plus size={14}/>
                </button>
                <button onClick={fetchProspects} style={{ padding:'8px 12px', background:'rgba(255,255,255,0.45)', color:'#6b6560', border:'1px solid rgba(0,0,0,0.08)', borderRadius:16, cursor:'pointer', flexShrink:0 }}>
                  <RefreshCw size={14}/>
                </button>
              </div>
              {/* Advanced filters toggle + saved searches */}
              <div className='flex gap-2 items-center'>
                <button onClick={() => setShowAdvancedFilters(!showAdvancedFilters)} style={{ padding:'6px 12px', borderRadius:100, border:'1px solid rgba(0,0,0,0.08)', background: showAdvancedFilters ? 'rgba(13,138,111,0.1)' : 'rgba(255,255,255,0.5)', color: showAdvancedFilters ? '#2DD4B8' : '#6b6560', fontFamily:"'Outfit', sans-serif", fontSize:11, cursor:'pointer', display:'flex', alignItems:'center', gap:4 }}>
                  <Target size={11}/> Filtres avancés {showAdvancedFilters ? '▲' : '▼'}
                </button>
                <button onClick={saveSearch} disabled={savingSearch} style={{ padding:'6px 12px', borderRadius:100, border:'1px solid rgba(0,0,0,0.08)', background:'rgba(255,255,255,0.5)', color:'#6b6560', fontFamily:"'Outfit', sans-serif", fontSize:11, cursor:'pointer' }}>
                  💾 Sauvegarder
                </button>
                {savedSearches.length > 0 && (
                  <select onChange={e => { const s = savedSearches.find(x => x.id === e.target.value); if (s) loadSearch(s); }} defaultValue='' style={{ padding:'6px 10px', borderRadius:100, border:'1px solid rgba(0,0,0,0.08)', background:'rgba(255,255,255,0.5)', fontFamily:"'Outfit', sans-serif", fontSize:11, color:'#6b6560', cursor:'pointer', outline:'none' }}>
              ...
              {showAdvancedFilters && (
                <div style={{ padding:12, background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:16, display:'flex', flexDirection:'column', gap:10 }}>
                  <div className='flex items-center gap-3 flex-wrap'>
                    <div style={{ flex:'1 1 200px' }}>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:10, color:'#9a9490', textTransform:'uppercase', letterSpacing:0.5, display:'block', marginBottom:4 }}>Score minimum: {scoreMin}</label>
                      <input type='range' min={0} max={100} value={scoreMin} onChange={e => setScoreMin(Number(e.target.value))} style={{ width:'100%', accentColor:'#2DD4B8' }} />
                    </div>
                    <div style={{ flex:'1 1 150px' }}>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:10, color:'#9a9490', textTransform:'uppercase', letterSpacing:0.5, display:'block', marginBottom:4 }}>Séquence</label>
                      <select value={sequenceFilter} onChange={e => setSequenceFilter(e.target.value as any)} style={{ width:'100%', padding:'6px 8px', background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:8, fontFamily:"'Outfit', sans-serif", fontSize:12, outline:'none' }}>
                    ...
                    <div style={{ flex:'1 1 150px' }}>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:10, color:'#9a9490', textTransform:'uppercase', letterSpacing:0.5, display:'block', marginBottom:4 }}>Dernière interaction (jours)</label>
                      <select value={lastInteractionDays ?? ''} onChange={e => setLastInteractionDays(e.target.value ? Number(e.target.value) : null)} style={{ width:'100%', padding:'6px 8px', background:'rgba(255,255,255,0.5)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:8, fontFamily:"'Outfit', sans-serif", fontSize:12, outline:'none' }}>
                  ...
                  {allSectors.length > 0 && (
                    <div>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:10, color:'#9a9490', textTransform:'uppercase', letterSpacing:0.5, display:'block', marginBottom:4 }}>Secteurs</label>
                      <div className='flex flex-wrap gap-1'>
                        {allSectors.map(s => (
                          <button key={s} onClick={() => setSectorFilter(prev => prev.includes(s) ? prev.filter(x => x !== s) : [...prev, s])} style={{ padding:'3px 10px', borderRadius:100, border:'1px solid', borderColor: sectorFilter.includes(s) ? '#2DD4B8' : 'rgba(0,0,0,0.08)', background: sectorFilter.includes(s) ? 'rgba(13,138,111,0.1)' : 'transparent', color: sectorFilter.includes(s) ? '#2DD4B8' : '#6b6560', fontFamily:"'Outfit', sans-serif", fontSize:11, cursor:'pointer' }}>{s}</button>
                        ))}
                      </div>
                    </div>
                  )}
                  {allTags.length > 0 && (
                    <div>
                      <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:10, color:'#9a9490', textTransform:'uppercase', letterSpacing:0.5, display:'block', marginBottom:4 }}>Tags</label>
                      <div className='flex flex-wrap gap-1'>
                        {allTags.map(t => (
                          <button key={t} onClick={() => setTagFilter(prev => prev.includes(t) ? prev.filter(x => x !== t) : [...prev, t])} style={{ padding:'3px 10px', borderRadius:100, border:'1px solid', borderColor: tagFilter.includes(t) ? '#A78BDB' : 'rgba(0,0,0,0.08)', background: tagFilter.includes(t) ? 'rgba(124,92,191,0.1)' : 'transparent', color: tagFilter.includes(t) ? '#A78BDB' : '#6b6560', fontFamily:"'Outfit', sans-serif", fontSize:11, cursor:'pointer' }}>{t}</button>
                        ))}
                      </div>
                    </div>
                  )}
            ...
            <div style={{ display:'flex', justifyContent:'flex-end' }}>
              ...
              ])} style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 14px', background:'rgba(255,255,255,0.45)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:11, cursor:'pointer', color:'#6b6560' }}>
            ...
            <div style={{ padding:'10px 14px', background:'rgba(212,165,90,0.08)', border:'1px solid rgba(212,165,90,0.3)', borderRadius:16, display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:8 }}>
              <span style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#6b6560', display:'flex', alignItems:'center', gap:4 }}>
                <Sparkles size={13} style={{ color:'#F0C95C' }}/>
                {stats.withEmail} / {stats.total} ont un email
              </span>
              <button onClick={findEmails} disabled={findingEmails} style={{ padding:'6px 14px', background:'#F0C95C', color:'#fff', border:'none', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:11, fontWeight:600, cursor:findingEmails?'not-allowed':'pointer', display:'flex', alignItems:'center', gap:4, opacity:findingEmails?0.7:1, whiteSpace:'nowrap' }}>
            ...
            <div style={{ padding:'10px 14px', background:'rgba(124,92,191,0.08)', border:'1px solid rgba(124,92,191,0.3)', borderRadius:16, display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:8 }}>
              ...
              <button onClick={findProspectInfo} disabled={findingInfo} style={{ padding:'6px 14px', background:'#A78BDB', color:'#fff', border:'none', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:11, fontWeight:600, cursor:findingInfo?'not-allowed':'pointer', display:'flex', alignItems:'center', gap:4, opacity:findingInfo?0.7:1, whiteSpace:'nowrap' }}>
            ...
            <div style={{ padding:'10px 14px', background:'rgba(59,130,246,0.08)', border:'1px solid rgba(59,130,246,0.3)', borderRadius:16, display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:8 }}>
              ...
              <button onClick={scoreProspects} disabled={scoring} style={{ padding:'6px 14px', background:'#3B82F6', color:'#fff', border:'none', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:11, fontWeight:600, cursor:scoring?'not-allowed':'pointer', display:'flex', alignItems:'center', gap:4, opacity:scoring?0.7:1, whiteSpace:'nowrap' }}>
            ...
            {selectedIds.size > 0 && (
              <div style={{ padding:'10px 14px', background:'rgba(13,138,111,0.08)', border:'1px solid rgba(13,138,111,0.3)', borderRadius:16, display:'flex', flexWrap:'wrap', alignItems:'center', gap:8 }}>
                <span style={{ fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#2DD4B8', fontWeight:600, marginRight:'auto' }}>{selectedIds.size} prospect(s) sélectionné(s)</span>
                <button onClick={() => setSelectedIds(new Set())} style={{ padding:'6px 12px', background:'transparent', border:'1px solid rgba(0,0,0,0.08)', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:11, color:'#6b6560', cursor:'pointer' }}>Désélectionner</button>
                <button onClick={deleteSelected} style={{ padding:'6px 12px', background:'#F07067', color:'#fff', border:'none', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:11, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:4 }}>
            ...
            {loading ? <div className='text-center py-20' style={{ color:'#9a9490', fontFamily:"'Outfit', sans-serif" }}>Chargement...</div>
            : filteredProspects.length === 0 ? <div className='text-center py-20' style={{ color:'#9a9490', fontFamily:"'Outfit', sans-serif" }}>Aucun prospect — utilise Chercher des prospects.</div>
            : (
              <div className='flex flex-col gap-6'>
                ...
                {prospectsNoSite.length > 0 && (
                  <div>
                    <div className='flex items-center gap-2 mb-3'>
                      ...
                      <h3 style={{ fontFamily:"'Playfair Display', serif", fontSize:16, color:'#2a2722', margin:0 }}>Sans site internet</h3>
                ...
                {prospectsWithSite.length > 0 && (
                  <div>
                    <div className='flex items-center gap-2 mb-3'>
                      <Globe size={16} style={{ color:'#9a9490' }}/>
                      <h3 style={{ fontFamily:"'Playfair Display', serif", fontSize:16, color:'#2a2722', margin:0 }}>Avec site internet</h3>
                      <span style={{ padding:'2px 10px', borderRadius:'100px', background:'rgba(255,255,255,0.5)', color:'#6b6560', fontFamily:"'Outfit', sans-serif", fontSize:12, fontWeight:600 }}>{prospectsWithSite.length}</span>
                    </div>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', backdropFilter:'blur(12px)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
          <div style={{ width:'100%', maxWidth:560, background:'rgba(255,255,255,0.18)', backdropFilter:'blur(24px) saturate(1.4)', borderRadius:28, padding:32, maxHeight:'90vh', overflowY:'auto', border:'1px solid rgba(255,255,255,0.35)', boxShadow:'0 8px 32px rgba(0,0,0,0.12)' }}>
            <div className='flex items-center justify-between mb-6'>
              <h2 style={{ fontFamily:"'Playfair Display', serif", fontSize:20, color:'#2a2722', margin:0 }}>Ajouter un prospect</h2>
              <button onClick={() => setShowManualAdd(false)} style={{ background:'none', border:'none', cursor:'pointer', color:'#9a9490' }}><X size={20}/></button>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
              {[
                ...
                <div key={field.key} style={{ gridColumn:field.full ? '1 / -1' : undefined }}>
                  <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#9a9490', display:'block', marginBottom:4 }}>{field.label}</label>
                  <input value={(manualForm as any)[field.key]} onChange={e => setManualForm(prev => ({ ...prev, [field.key]: e.target.value }))} style={{ width:'100%', padding:'10px 14px', background:'rgba(255,255,255,0.85)', border:'1px solid rgba(255,255,255,0.5)', borderRadius:12, fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#2a2722', outline:'none', boxSizing:'border-box', boxShadow:'inset 0 1px 2px rgba(0,0,0,0.04)' }} />
                </div>
              ))}
            </div>
            <div className='flex gap-3 mt-6'>
              <button onClick={() => setShowManualAdd(false)} style={{ flex:1, padding:12, background:'rgba(255,255,255,0.25)', border:'1px solid rgba(255,255,255,0.4)', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:14, cursor:'pointer', color:'#6b6560', backdropFilter:'blur(8px)' }}>Annuler</button>
              <button onClick={handleManualAdd} style={{ flex:1, padding:12, background:'#2DD4B8', color:'#fff', border:'none', borderRadius:100, fontFamily:"'Outfit', sans-serif", fontSize:14, fontWeight:600, cursor:'pointer' }}>Ajouter</button>
            </div>
          </div>
        </div>
      )}
...
const ProspectTable = ({ prospects, selectedIds, onToggleSelect, onToggleSelectAll, onDelete, onUpdateEmail, onUpdateStatus, onViewDetail }: {
  ...
  <div style={{ background:'rgba(255,255,255,0.45)', backdropFilter:'blur(20px)', borderRadius:'12px', border:'1px solid rgba(0,0,0,0.08)', overflow:'hidden' }}>
        ...
        <div key={p.id} className='flex items-start gap-3 p-4' style={{ borderBottom:'1px solid rgba(0,0,0,0.04)', cursor:'pointer' }} onClick={() => onViewDetail(p)}>
          ...
          <div style={{ flex:1 }}>
            <div className='flex items-center gap-2 mb-1'>
              <span style={{ fontWeight:600, color:'#2a2722', fontFamily:"'Outfit', sans-serif" }}>{p.business_name}</span>
            ...
            <p style={{ fontSize:12, color:'#6b6560', margin:'2px 0', fontFamily:"'Outfit', sans-serif" }}>{p.email || 'Pas d\'email'}</p>
            <p style={{ fontSize:12, color:'#9a9490', margin:0, fontFamily:"'Outfit', sans-serif" }}>{p.city} - {p.has_website ? 'Site OK' : 'Sans site'} - {p.email_count}x</p>
          </div>
        </div>
      ))}
    </div>
...
const ProspectRow = ({ prospect: p, selected, onToggle, onDelete, onUpdateEmail, onUpdateStatus, onViewDetail }: {
  ...
  return (
    <tr style={{ borderBottom:'1px solid rgba(0,0,0,0.04)', background:selected?'rgba(13,138,111,0.04)':'transparent', cursor:'pointer' }} onClick={onViewDetail}>
      ...
      <td className='px-4 py-3'>
        <div className='flex items-center gap-2'>
          <div style={{ fontWeight:600, color:'#2a2722', fontSize:14 }}>{p.business_name}</div>
        ...
        <div className='flex items-center gap-1'>
          {p.business_type && <span style={{ fontSize:11, color:'#9a9490' }}>{p.business_type}</span>}
          {p.sector && p.sector !== p.business_type && <span style={{ fontSize:10, color:'#2DD4B8', fontWeight:600 }}>• {p.sector}</span>}
        </div>
      </td>
      <td className='px-4 py-3' style={{ color:'#6b6560', fontSize:13 }}>
        {p.contact_name || '—'}
        {p.phone && <div style={{ fontSize:11, color:'#9a9490' }}>{p.phone}</div>}
      </td>
      <td className='px-4 py-3'>
        {editingEmail ? (
          <div className='flex items-center gap-1'>
            <input value={emailVal} onChange={e => setEmailVal(e.target.value)} autoFocus onKeyDown={e => { if(e.key==='Enter'){onUpdateEmail(emailVal);setEditingEmail(false);} if(e.key==='Escape')setEditingEmail(false); }} style={{ padding:'4px 8px', border:'1px solid #2DD4B8', borderRadius:6, fontSize:13, outline:'none', width:160 }} />
            <button onClick={() => { onUpdateEmail(emailVal); setEditingEmail(false); }} style={{ background:'none', border:'none', cursor:'pointer', color:'#2DD4B8' }}><Check size={14}/></button>
            <button onClick={() => setEditingEmail(false)} style={{ background:'none', border:'none', cursor:'pointer', color:'#9a9490' }}><X size={14}/></button>
          </div>
        ) : (
          <div className='flex items-center gap-2'>
      ...
      <td className='px-4 py-3' style={{ color:'#6b6560', fontSize:13 }}>
        <div>{p.city || '—'}</div>
        <span style={{ fontSize:10, color:'#9a9490', fontWeight:600 }}>{LANG_LABELS[p.language || COUNTRY_LANG[p.country || ''] || 'en'] || p.language}</span>
      </td>
      <td className='px-4 py-3'>
        {p.has_website
          ? <span style={{ display:'flex', alignItems:'center', gap:4, fontSize:12, color:'#9a9490' }}><Globe size={12}/> A un site</span>
...
const BulkEmailModal = ({ prospects, generatedEmails, generatingAll, sending, onUpdateEmail, onSend, onClose }: {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', backdropFilter:'blur(8px)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ width:'100%', maxWidth:800, background:'rgba(255,255,255,0.5)', borderRadius:'28px', overflow:'hidden', maxHeight:'92vh', display:'flex', flexDirection:'column' }}>
        <div style={{ padding:'20px 28px', borderBottom:'1px solid rgba(0,0,0,0.08)', display:'flex', alignItems:'center', justifyContent:'space-between', background:'rgba(255,255,255,0.45)' }}>
          <div>
            <h2 style={{ fontFamily:"'Playfair Display', serif", fontSize:18, color:'#2a2722', margin:0 }}>Emails generes par Claude AI</h2>
            <p style={{ fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#6b6560', margin:'4px 0 0' }}>
              {generatingAll ? 'Generation en cours...' : ready.length + '/' + prospects.length + ' prets'}
            </p>
          </div>
          <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', color:'#6b6560' }}><X size={20}/></button>
        </div>
        <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
          <div style={{ width:200, borderRight:'1px solid rgba(0,0,0,0.08)', overflowY:'auto', background:'rgba(255,255,255,0.5)', flexShrink:0 }}>
            {prospects.map((p,i) => {
          ...
          <div style={{ flex:1, overflowY:'auto', padding:28 }}>
            {generatedEmails[activeIdx] && (() => {
              ...
              if (e.loading) return (
                <div style={{ display:'flex', alignItems:'center', gap:12, color:'#6b6560', fontFamily:"'Outfit', sans-serif", paddingTop:40 }}>
                  <Loader2 size={20} className='animate-spin' style={{ color:'#2DD4B8' }}/>
                  Generation pour {p.business_name}...
                </div>
              );
              if (e.error) return (
                <div style={{ padding:20, background:'rgba(232,115,90,0.1)', border:'1px solid rgba(232,115,90,0.3)', borderRadius:'12px', color:'#F07067', fontFamily:"'Outfit', sans-serif", fontSize:14 }}>
                  Erreur: {e.error}
                </div>
              );
              return (
                <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                  <div style={{ padding:'8px 12px', background:'rgba(13,138,111,0.06)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#6b6560' }}>
                    <strong>A:</strong> {p.email} &nbsp; <strong>Commerce:</strong> {p.business_name} &nbsp; {p.city}
                  </div>
                  <div>
                    <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#9a9490', display:'block', marginBottom:6 }}>OBJET</label>
                    <input value={e.subject} onChange={ev => onUpdateEmail(activeIdx,'subject',ev.target.value)} style={{ width:'100%', padding:'10px 14px', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:14, color:'#2a2722', outline:'none', boxSizing:'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontFamily:"'Outfit', sans-serif", fontSize:12, color:'#9a9490', display:'block', marginBottom:6 }}>CORPS EMAIL (HTML editable)</label>
                    <textarea value={e.body} onChange={ev => onUpdateEmail(activeIdx,'body',ev.target.value)} rows={14} style={{ width:'100%', padding:'12px 14px', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'12px', fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#2a2722', outline:'none', resize:'vertical', lineHeight:1.7, boxSizing:'border-box' }} />
                  </div>
                </div>
              );
            })()}
        ...
        <div style={{ padding:'16px 28px', borderTop:'1px solid rgba(0,0,0,0.08)', display:'flex', alignItems:'center', justifyContent:'space-between', background:'rgba(255,255,255,0.5)' }}>
          <p style={{ fontFamily:"'Outfit', sans-serif", fontSize:13, color:'#6b6560', margin:0 }}>{ready.length} email(s) prets</p>
          <div className='flex gap-3'>
            <button onClick={onClose} style={{ padding:'10px 20px', background:'transparent', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'100px', fontFamily:"'Outfit', sans-serif", fontSize:13, cursor:'pointer', color:'#6b6560' }}>Annuler</button>
            <button onClick={onSend} disabled={sending||generatingAll||!ready.length} style={{ padding:'10px 24px', background:'#2DD4B8', color:'#fff', border:'none', borderRadius:'100px', fontFamily:"'Outfit', sans-serif", fontSize:13, fontWeight:600, cursor:(sending||generatingAll||!ready.length)?'not-allowed':'pointer', opacity:(sending||generatingAll||!ready.length)?0.6:1, display:'flex', alignItems:'center', gap:8 }}>
