...
import { RefreshCw, Search, Filter, FileText, AlertCircle } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const updateLeadStatus = async (leadId: string, status: string) => {
    ...
    if (error) toast.error('Erreur mise à jour');
      ...
      fetchLeads();
      if (selectedLead?.id === leadId) setSelectedLead(prev => prev ? { ...prev, status: status as Lead['status'] } : null);
    }
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--cream)' }}>
      ...
      <div className="p-6 max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="flex items-center gap-2 flex-1" style={{ padding: '10px 16px', background: 'var(--glass-bg)', borderRadius: 'var(--r)', border: '1px solid var(--glass-border)' }}>
            ...
            <input placeholder="Rechercher un prospect..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', fontFamily: 'var(--font-b)', fontSize: 14, color: 'var(--text)' }} />
          </div>
          <div className="flex items-center gap-2" style={{ padding: '10px 16px', background: 'var(--glass-bg)', borderRadius: 'var(--r)', border: '1px solid var(--glass-border)' }}>
            ...
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ background: 'transparent', border: 'none', outline: 'none', fontFamily: 'var(--font-b)', fontSize: 14, color: 'var(--text)', cursor: 'pointer' }}>
        ...
        {loading ? (
          ...
          <div className="text-center py-20" style={{ color: 'var(--text-light)', fontFamily: 'var(--font-b)' }}>Aucun prospect trouvé</div>
        ) : (
          <div style={{ background: 'var(--glass-bg-strong)', backdropFilter: 'blur(20px)', borderRadius: 'var(--r)', border: '1px solid var(--glass-border)', overflow: 'hidden' }}>
              ...
              <table className="w-full" style={{ fontFamily: 'var(--font-b)', fontSize: 14 }}>
                ...
                <tbody>
                  {filteredLeads.map(lead => (
                    <tr key={lead.id} className="cursor-pointer transition-colors" style={{ borderBottom: '1px solid rgba(0,0,0,0.04)' }} onClick={() => setSelectedLead(lead)} onMouseEnter={e => e.currentTarget.style.background = 'rgba(13,138,111,0.04)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                      <td className="px-4 py-3 font-medium" style={{ color: 'var(--charcoal)' }}>{lead.name}</td>
                      <td className="px-4 py-3" style={{ color: 'var(--text-mid)' }}>{lead.email}</td>
                      <td className="px-4 py-3" style={{ color: 'var(--text-mid)' }}>{lead.project_type || '—'}</td>
                      <td className="px-4 py-3" style={{ color: 'var(--text-mid)' }}>{lead.budget || '—'}</td>
                      <td className="px-4 py-3">
                        <select value={lead.status} onClick={e => e.stopPropagation()} onChange={e => { e.stopPropagation(); updateLeadStatus(lead.id, e.target.value); }} style={{ padding: '4px 8px', borderRadius: 'var(--pill)', border: 'none', background: `${statusColors[lead.status]}18`, color: statusColors[lead.status], fontWeight: 600, fontSize: 12, fontFamily: 'var(--font-m)', cursor: 'pointer' }}>
                          {Object.entries(statusLabels).map(([key, label]) => (
                            <option key={key} value={key}>{label}</option>
                          ))}
                        </select>
                      </td>
                      <td className="px-4 py-3" style={{ color: 'var(--text-light)', fontSize: 12 }}>{new Date(lead.created_at).toLocaleDateString('fr-FR')}</td>
                      <td className="px-4 py-3">
                        <button onClick={e => { e.stopPropagation(); setEmailLead(lead); }} style={{ padding: '6px 12px', background: 'var(--teal)', color: '#fff', border: 'none', borderRadius: 'var(--pill)', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-b)' }}>Email</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="md:hidden flex flex-col">
              {filteredLeads.map(lead => (
                <div key={lead.id} className="p-4 cursor-pointer" style={{ borderBottom: '1px solid rgba(0,0,0,0.04)' }} onClick={() => setSelectedLead(lead)}>
                  <div className="flex items-center justify-between mb-2">
                    <span style={{ fontWeight: 600, color: 'var(--charcoal)', fontFamily: 'var(--font-b)' }}>{lead.name}</span>
                    <span style={{ padding: '3px 10px', borderRadius: 'var(--pill)', background: `${statusColors[lead.status]}18`, color: statusColors[lead.status], fontWeight: 600, fontSize: 11, fontFamily: 'var(--font-m)' }}>{statusLabels[lead.status]}</span>
                  </div>
                  <p style={{ fontSize: 13, color: 'var(--text-mid)', fontFamily: 'var(--font-b)' }}>{lead.email}</p>
                  <p style={{ fontSize: 12, color: 'var(--text-light)', fontFamily: 'var(--font-b)', marginTop: 4 }}>{lead.project_type || '—'} • {lead.budget || '—'} • {new Date(lead.created_at).toLocaleDateString('fr-FR')}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {selectedLead && (
        <LeadDetail lead={selectedLead} onClose={() => setSelectedLead(null)} onStatusChange={(status) => updateLeadStatus(selectedLead.id, status)} onSendEmail={() => setEmailLead(selectedLead)} />
      )}
      {emailLead && (
        <SendEmailModal lead={emailLead} onClose={() => setEmailLead(null)} onSent={() => { setEmailLead(null); fetchLeads(); }} />
      )}
    </div>
  );
};
