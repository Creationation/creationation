...
const InvoiceFormModal = ({ invoice, clients, onClose, onSaved, prefillClientId, prefillProjectId }: Props) => {
  ...
  const labelStyle: React.CSSProperties = {
    fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: 600, color: '#6b6560', marginBottom: 4, display: 'block',
  };
    ...
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9a9490' }}><X size={20} /></button>
        </div>
        ...
        {/* Section 2 — Items */}
        <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 14, fontWeight: 600, color: '#2a2722', marginBottom: 12 }}>Lignes de facturation</h3>
        <div className="overflow-x-auto mb-2">
          <table className="w-full" style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
                <th className="text-left py-2 px-2" style={{ color: '#9a9490', fontSize: 11, fontWeight: 600 }}>Description</th>
                <th className="text-left py-2 px-2" style={{ color: '#9a9490', fontSize: 11, fontWeight: 600, width: 80 }}>Qté</th>
                <th className="text-left py-2 px-2" style={{ color: '#9a9490', fontSize: 11, fontWeight: 600, width: 110 }}>Prix unit.</th>
                <th className="text-left py-2 px-2" style={{ color: '#9a9490', fontSize: 11, fontWeight: 600, width: 100 }}>Total</th>
            ...
            <tbody>
              {items.map((it, idx) => (
                <tr key={idx} style={{ borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                  ...
                  <td className="py-2 px-2" style={{ fontWeight: 600, color: '#2a2722' }}>{fmt(it.total)}</td>
                  <td className="py-2 px-2">
                    {items.length > 1 && (
        ...
        <button onClick={() => setItems(prev => [...prev, { description: '', quantity: 1, unit_price: 0, total: 0 }])}
          className="flex items-center gap-1 mb-4" style={{
            padding: '6px 12px', background: 'rgba(255,255,255,0.5)', border: '1px solid rgba(0,0,0,0.08)',
        ...
        <div className="flex flex-col items-end gap-2 mb-6" style={{ fontFamily: "'Outfit', sans-serif", fontSize: 14 }}>
          <div className="flex gap-8">
            <span style={{ color: '#6b6560' }}>Sous-total</span>
          ...
          <div className="flex items-center gap-4">
            ...
            <input type="number" min={0} max={100} step={0.5} value={taxRate} onChange={e => setTaxRate(Number(e.target.value))}
              style={{ width: 60, padding: '4px 8px', background: 'rgba(255,255,255,0.5)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 8, textAlign: 'center', fontSize: 13, fontFamily: "'Outfit', sans-serif" }} />
            <span>%</span>
            <span style={{ fontWeight: 600 }}>{fmt(taxAmount)}</span>
          </div>
          <div className="flex gap-8" style={{ fontSize: 18, fontWeight: 700, color: '#2a2722' }}>
            <span>Total TTC</span>
            <span>{fmt(total)}</span>
          </div>
        </div>
        ...
        <div className="flex gap-3 justify-end">
          <button onClick={() => handleSave(true)} disabled={saving} style={{
            padding: '12px 24px', background: 'transparent', border: '2px solid rgba(0,0,0,0.08)',
