      ...
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 640, background: 'white', borderRadius: 28, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ padding: '24px 24px 16px', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
          <div className="flex items-center justify-between">
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, color: '#2a2722', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, padding: '16px 24px' }}>
          {[
            { label: 'Total prospects', value: totalProspects, icon: Users, color: '#2a2722' },
            ...
            <div key={k.label} style={{ padding: '12px', background: 'rgba(255,255,255,0.5)', borderRadius: 16, border: '1px solid rgba(0,0,0,0.08)', textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, color: '#9a9490', textTransform: 'uppercase', letterSpacing: 0.5 }}>{k.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: '0 24px 24px' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: 40, color: '#9a9490', fontFamily: "'Outfit', sans-serif" }}>Aucun prospect trouvé</div>
          ) : (
            <div className="flex flex-col gap-2">
              {stats.map(s => {
                ...
                return (
                  <div key={s.sector} style={{ background: 'rgba(255,255,255,0.5)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 16, overflow: 'hidden' }}>
                    <button
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 14, fontWeight: 600, color: '#2a2722' }}>{s.label}</div>
                        <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, color: '#9a9490' }}>
                          {s.total} prospects · {s.converted} convertis · {rate}%
                        </div>
                      </div>
                      {/* Mini bar */}
                      <div style={{ width: 60, height: 6, background: 'rgba(0,0,0,0.08)', borderRadius: 3, overflow: 'hidden' }}>
                        <div style={{ width: `${Math.min(100, parseFloat(rate))}%`, height: '100%', background: '#2DD4B8', borderRadius: 3 }} />
                      </div>
                      {isExpanded ? <ChevronUp size={16} style={{ color: '#9a9490' }} /> : <ChevronDown size={16} style={{ color: '#9a9490' }} />}
                    </button>
                    {isExpanded && (
                      <div style={{ padding: '0 16px 16px', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                        {[
                          ...
                          <div key={m.label} style={{ padding: '8px 12px', background: 'rgba(13,138,111,0.04)', borderRadius: 12 }}>
                            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, color: '#2a2722' }}>{m.value}</div>
                            <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, color: '#9a9490', textTransform: 'uppercase' }}>{m.label}</div>
                          </div>
                        ))}
                      </div>
                    )}
